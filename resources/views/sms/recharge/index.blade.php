        ...
        <form method="post" action="{{ route('sms.recharge.create') }}" class="recharge-card" data-recharge-form>
            ...
            <div class="recharge-modal" data-recharge-modal hidden>
                ...
                <div class="recharge-modal-card" role="dialog" aria-modal="true" aria-labelledby="recharge-modal-title">
                    ...
                    <div class="modal-step-confirm" data-modal-confirm>
                        <div class="modal-kicker">{{ __('sms.recharge.show_eyebrow') }}</div>
                        <h2 id="recharge-modal-title">{{ __('sms.recharge.confirm_title') }}</h2>
                        <p>{{ __('sms.recharge.confirm_sub') }}</p>

                        <div class="modal-summary-grid compact-summary-grid">
                            <div><span>{{ __('sms.recharge.pay_amount') }}</span><b data-modal-preview-pay>{{ $firstPlan ? '¥'.number_format((float)$firstPlan->amount, 2) : '¥--' }}</b></div>
                            <div><span>{{ __('sms.recharge.credit_amount') }}</span><b data-modal-preview-credit>{{ $firstPlan ? '¥'.number_format((float)$firstPlan->total_amount, 2) : '¥--' }}</b></div>
                        </div>

                        <div class="modal-method-block">
                            <h3>{{ __('sms.recharge.select_payment_method') }}</h3>
                            @if(empty($methods))
                    ...
                    <div class="modal-step-created" data-modal-created hidden>
                        <div class="modal-kicker">{{ __('sms.recharge.show_eyebrow') }}</div>
                        ...
                        <div class="modal-actions">
                            <button class="btn btn-dark" type="button" data-reset-modal>{{ __('sms.recharge.change_selection') }}</button>
...
(function(){
    ...
    const openModalButton = document.querySelector('[data-open-recharge-modal]');
    const submit = document.querySelector('[data-submit-text]');
    const errorBox = document.querySelector('[data-recharge-error]');
    const modalErrorBox = document.querySelector('[data-modal-error]');
    ...
    function currentPlan() {
        return planCards.find(card => card.querySelector('input[type="radio"]:checked')) || planCards[0];
    }
    ...
    function showError(message) {
        [errorBox, modalErrorBox].forEach(box => {
            if (!box) return;
    ...
    function showCreated(order) {
        modalConfirm.hidden = true;
        modalCreated.hidden = false;
    ...
    document.querySelectorAll('[data-reset-modal]').forEach(item => item.addEventListener('click', () => { modalConfirm.hidden = false; modalCreated.hidden = true; }));
