...
class SmsAdminController extends Controller
{
    ...
    public function homeSlides()
    {
        if (SmsHomeSlide::count() === 0) {
            foreach ([
                ['badge' => '长效接码号', 'title' => '60 天长效接码', 'description' => '主售长期可用号码，站内购买后自动交付；手机号、短信和订单记录都在账号里长期可查。', 'image_url' => '/images/home/slide-1.webp', 'card_title' => '号码有效期', 'card_value' => '60 天', 'card_description' => '适合长期账号验证', 'sort_order' => 10, 'translations' => ['zh_CN' => ['badge' => '长效接码号', 'title' => '60 天长效接码', 'description' => '主售长期可用号码，站内购买后自动交付；手机号、短信和订单记录都在账号里长期可查。', 'card_title' => '号码有效期', 'card_value' => '60 天', 'card_description' => '适合长期账号验证'], 'en' => ['badge' => 'Long-term SMS', 'title' => '60-day SMS numbers', 'description' => 'We focus on long-validity numbers. After purchase, the number, messages and order history stay available inside your account.', 'card_title' => 'Validity', 'card_value' => '60 Days', 'card_description' => 'Built for long-term verification']]],
                ['badge' => '余额工作台', 'title' => '余额支付，自动交付', 'description' => '充值后用账户余额下单，成功后自动发放号码；库存不足、发货失败或异常会自动退回余额。', 'image_url' => '/images/home/slide-2.webp', 'card_title' => '交付方式', 'card_value' => '自动', 'card_description' => '购买 → 发货 → 收码', 'sort_order' => 20, 'translations' => ['zh_CN' => ['badge' => '余额工作台', 'title' => '余额支付，自动交付', 'description' => '充值后用账户余额下单，成功后自动发放号码；库存不足、发货失败或异常会自动退回余额。', 'card_title' => '交付方式', 'card_value' => '自动', 'card_description' => '购买 → 发货 → 收码'], 'en' => ['badge' => 'Balance Workspace', 'title' => 'Balance payment, auto delivery', 'description' => 'Top up once and pay with account balance. Numbers are delivered automatically, and failed delivery returns balance automatically.', 'card_title' => 'Delivery', 'card_value' => 'Auto', 'card_description' => 'Buy → deliver → receive']]],
                ['badge' => '订单追踪', 'title' => '订单可查，记录可追踪', 'description' => '登录账号可查看全部号码，也可以通过下单邮箱或订单号找回记录，客服和交流群入口统一配置。', 'image_url' => '/images/home/slide-3.webp', 'card_title' => '记录保存', 'card_value' => '长期', 'card_description' => '邮箱与订单号均可查询', 'sort_order' => 30, 'translations' => ['zh_CN' => ['badge' => '订单追踪', 'title' => '订单可查，记录可追踪', 'description' => '登录账号可查看全部号码，也可以通过下单邮箱或订单号找回记录，客服和交流群入口统一配置。', 'card_title' => '记录保存', 'card_value' => '长期', 'card_description' => '邮箱与订单号均可查询'], 'en' => ['badge' => 'Order Tracking', 'title' => 'Searchable orders and records', 'description' => 'View all numbers after login, or recover records with order email or order number. Support and community links are configured in admin.', 'card_title' => 'Records', 'card_value' => 'Saved', 'card_description' => 'Recover by email or order ID']]],
            ] as $slide) {
    ...
    private function slideData(Request $request)
    {
        $data = $request->validate([
            ...
            'translations.*.badge' => ['nullable', 'string', 'max:80'],
            'translations.*.title' => ['nullable', 'string', 'max:160'],
            'translations.*.description' => ['nullable', 'string', 'max:1000'],
            'translations.*.card_title' => ['nullable', 'string', 'max:80'],
            'translations.*.card_value' => ['nullable', 'string', 'max:80'],
            'translations.*.card_description' => ['nullable', 'string', 'max:160'],
        ]);

        $translations = $this->slideTranslations($request);
        $fallback = $this->fallbackSlideCopy($translations);

        $data['badge'] = $fallback['badge'] ?? null;
        $data['title'] = $fallback['title'] ?? '未命名轮播';
        $data['description'] = $fallback['description'] ?? null;
        $data['card_title'] = $fallback['card_title'] ?? null;
        $data['card_value'] = $fallback['card_value'] ?? null;
        $data['card_description'] = $fallback['card_description'] ?? null;
    ...
    private function slideTranslations(Request $request): array
    {
        $supported = array_keys(config('sms.locale.supported', []));
        $input = $request->input('translations', []);
        $fields = ['badge', 'title', 'description', 'card_title', 'card_value', 'card_description'];
        $translations = [];
    ...
    private function fallbackSlideCopy(array $translations): array
    {
        $default = config('sms.locale.default', 'zh_CN');
        if (! empty($translations[$default]['title'])) {
            return $translations[$default];
        }

        foreach ($translations as $copy) {
            if (! empty($copy['title'])) {
                return $copy;
            }
        }
